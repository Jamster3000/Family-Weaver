<script lang="ts">
	...
	let entries = $derived(
		categoryKey && $personData ? $personData[categoryKey] ?? [] : []
	);

	let displayTitle = $derived(categoryTitle?.toLowerCase() ?? "this category");
			...
			{#if entries.length === 0}
				<Card width="100%" padding="medium">
					<div class="empty-state">
						...
						<p class="empty-sub">
							Click "+ Add Entry" to record a milestone for {displayTitle}.
						</p>
					</div>
				</Card>
			{:else}
